...
{
  HOU_EXPECT_PRECONDITION(RenderCanvas rs(Vec2u(0u, 0u), 0u));
  HOU_EXPECT_PRECONDITION(RenderCanvas rs(Vec2u(1u, 0u), 0u));
  HOU_EXPECT_PRECONDITION(RenderCanvas rs(Vec2u(0u, 1u), 0u));
}
...
TEST_F(TestRenderCanvasDeathTest, CreationSizeTooLarge)
{
  ...
  HOU_EXPECT_PRECONDITION(RenderCanvas rs(Vec2u(size, size), 0u));
  HOU_EXPECT_PRECONDITION(RenderCanvas rs(Vec2u(1u, size), 0u));
  HOU_EXPECT_PRECONDITION(RenderCanvas rs(Vec2u(size, 1u), 0u));
}
...
{
  RenderCanvas rs1(Vec2u(3u, 4u), 0u);
  RenderCanvas rs2(Vec2u(3u, 4u), 1u);
  RenderCanvas rs3(Vec2u(3u, 4u), 2u);
...
{
  Vec2u size(3u, 4u);
  RenderCanvas rsSrc(size, 0u);
  RenderCanvas rsDst(size, 0u);
...
{
  RenderCanvas rs1(Vec2u(3u, 4u), 0u);
  RenderCanvas rs2(Vec2u(3u, 4u), 0u);
