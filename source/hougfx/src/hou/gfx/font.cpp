...
ft_library_wrapper::ft_library_wrapper()
  ...
  , library(nullptr)
{
  DEPRECATED_HOU_ENSURE(FT_Init_FreeType(&library) == 0);
  DEPRECATED_HOU_ENSURE_DEV(library != nullptr);
}
...
{
  DEPRECATED_HOU_EXPECT_DEV(m_face != nullptr);
...
void font::set_face_index(uint face_index)
{
  ...
  {
    DEPRECATED_HOU_EXPECT_DEV(m_face != nullptr);
    DEPRECATED_HOU_EXPECT(face_index < get_face_index_count());
...
void font::set_pixel_height(uint pixel_height)
{
  if(m_pixel_height != pixel_height)
  {
    ...
    DEPRECATED_HOU_ENSURE(FT_Set_Pixel_Sizes(m_face, 0, pixel_height) == 0);
  }
}
...
{
  DEPRECATED_HOU_EXPECT_DEV(m_face != nullptr);
  DEPRECATED_HOU_ENSURE(FT_Load_Char(m_face, char_code, FT_LOAD_RENDER) == 0);
...
vec2i font::get_kerning(utf32::code_unit first, utf32::code_unit second) const
{
  ...
  DEPRECATED_HOU_ENSURE(FT_Get_Kerning(
...
void font::load()
{
  ...
  DEPRECATED_HOU_ENSURE(FT_Set_Pixel_Sizes(m_face, 0, m_pixel_height) == 0);
  DEPRECATED_HOU_ENSURE(FT_Select_Charmap(m_face, FT_ENCODING_UNICODE) == 0);
}
