...
{
  HOU_DEV_PRECOND(m_color_attachment != nullptr);
...
{
  HOU_DEV_POSTCOND(graphic_context::get_rendering_color_byte_count() == 4u);
  HOU_DEV_POSTCOND(graphic_context::get_rendering_depth_byte_count() == 3u);
  HOU_DEV_POSTCOND(graphic_context::get_rendering_stencil_byte_count() == 1u);
  ...
  HOU_DEV_POSTCOND(m_color_attachment != nullptr);
  HOU_DEV_POSTCOND(m_depth_stencil_attachment != nullptr);
  ...
  HOU_DEV_POSTCOND(m_framebuffer.is_complete());
}
