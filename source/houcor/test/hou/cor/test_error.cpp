...
TEST_F(test_error, format_error_message)
{
...
{
  EXPECT_DEATH(
    HOU_TERMINATE(u8"This is a fatal error!"), u8"This is a fatal error!");
}
...
{
  EXPECT_DEATH(
    HOU_FATAL_ERROR(u8"This is a %s error!%d!", "bad", 1),
...
{
  HOU_EXPECT_ERROR(
    HOU_RUNTIME_ERROR(u8"This is a %s error!", u8"runtime"), std::runtime_error,
...
{
  HOU_EXPECT_ERROR(
    HOU_LOGIC_ERROR(u8"This is a %s error!", u8"logic"), std::logic_error,
...
{
  HOU_EXPECT_ERROR(
    HOU_EXPECT(2 < 1), std::logic_error,
...
#else
  HOU_EXPECT_ERROR(
    HOU_EXPECT_DEV(2 < 1), std::logic_error,
    format_string(get_text(cor_error::pre_condition), "2 < 1"));
#endif
}
...
{
  HOU_EXPECT_ERROR(
    HOU_ENSURE(2 < 1), std::logic_error,
...
#else
  HOU_EXPECT_ERROR(
    HOU_ENSURE_DEV(2 < 1), std::logic_error,
    format_string(get_text(cor_error::post_condition), "2 < 1"));
#endif
}
