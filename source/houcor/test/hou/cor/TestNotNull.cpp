...
// Not: NotNull clashes with a gtest defined class. Therefore it is necessary to
...
TEST_F(TestNotNull, UniquePtrConstruction)
{
  ...
  hou::NotNull<IntPtr> nnp(std::make_unique<int>(2));
  EXPECT_EQ(2, *nnp.get());
  EXPECT_EQ(2, *nnp);
}
...
TEST_F(TestNotNull, UniquePtrClassMethodDereferencing)
{
  ...
  hou::NotNull<FooPtr> nnp(std::make_unique<Foo>());
...
TEST_F(TestNotNull, SharedPtrGet)
{
  ...
  hou::NotNull<IntPtr> nnp(std::make_shared<int>(2));
  IntPtr p = nnp.get();
  EXPECT_EQ(2, *p);
}
...
TEST_F(TestNotNull, SharedPtrClassMethodDereferencing)
{
  ...
  hou::NotNull<FooPtr> nnp(std::make_shared<Foo>());
  EXPECT_EQ(0, nnp->getInt());
  nnp->setInt(42);
  EXPECT_EQ(42, nnp->getInt());
}
