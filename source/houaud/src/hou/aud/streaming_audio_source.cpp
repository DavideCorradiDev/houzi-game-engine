...
{
  DEPRECATED_HOU_EXPECT(buffer_count > 0u);
  stop();
  std::lock_guard<std::mutex> lock(m_thread_mutex);
  free_buffers();
...
{
  DEPRECATED_HOU_EXPECT(buffer_sample_count > 0u);
  stop();
  std::lock_guard<std::mutex> lock(m_thread_mutex);
...
size_t streaming_audio_source::buffer_queue::free_buffers(size_t count)
{
  ...
  DEPRECATED_HOU_ENSURE_DEV(m_free_buffer_count <= m_buffers.size());
...
{
  DEPRECATED_HOU_EXPECT_DEV(m_free_buffer_count > 0);
