...
void bind_buffer(const buffer_handle& buffer, GLenum target)
{
  ...
  if(!is_buffer_bound(buffer, target))
  {
    ...
    HOU_GL_CHECK_ERROR();
    context::getCurrent()->mTrackingData.set_bound_buffer(buffer.get_uid()
      , target);
  }
}
...
void unbind_buffer(GLenum target)
{
  ...
  if(is_buffer_bound(target))
  {
    ...
    HOU_GL_CHECK_ERROR();
    context::getCurrent()->mTrackingData.set_bound_buffer(0u, target);
  }
}
...
bool is_buffer_bound(const buffer_handle& buffer, GLenum target)
{
  HOU_GL_CHECK_CONTEXT_EXISTENCE();
  HOU_GL_CHECK_CONTEXT_OWNERSHIP(buffer);
  return context::getCurrent()->mTrackingData.get_bound_buffer(target)
    == buffer.get_uid();
}
...
{
  HOU_GL_CHECK_CONTEXT_EXISTENCE();
  return context::getCurrent()->mTrackingData.get_bound_buffer(target) != 0u;
}
