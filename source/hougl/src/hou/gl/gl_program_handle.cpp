...
void bind_program(const program_handle& program)
{
  ...
  if(!is_program_bound(program))
  {
    ...
    HOU_GL_CHECK_ERROR();
    context::getCurrent()->mTrackingData.set_bound_program(program.get_uid());
  }
}
...
void unbind_program()
{
  ...
  if(is_program_bound())
  {
    glUseProgram(0u);
    HOU_GL_CHECK_ERROR();
    context::getCurrent()->mTrackingData.set_bound_program(0u);
  }
}
...
bool is_program_bound(const program_handle& program)
{
  HOU_GL_CHECK_CONTEXT_EXISTENCE();
  HOU_GL_CHECK_CONTEXT_OWNERSHIP(program);
  return context::getCurrent()->mTrackingData.get_bound_program()
    == program.get_uid();
}
...
{
  HOU_GL_CHECK_CONTEXT_EXISTENCE();
  return context::getCurrent()->mTrackingData.get_bound_program() != 0u;
}
